...
export const mockUsers: User[] = [
  ...
  },
  {
    ...
    password: '$2b$10$6WE6Mdly3gJCdo1G/s5vUuj7gxdmnLvuUiouSEOxHeYME56ezEDQS', // hashed 'admin123'
    name: 'Admin User',
    role: 'admin',
...
import { sendVerificationEmail as sendRealEmail } from './email-service';

// Function to send verification email (uses real email service with mock fallback)
export async function sendVerificationEmail(email: string, token: string, name: string): Promise<boolean> {
  return await sendRealEmail(email, token, name);
}