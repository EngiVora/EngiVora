...
export function AdminDashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const [usersRes, blogsRes, jobsRes] = await Promise.all([
        fetch('/api/students/list?limit=1', { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch('/api/blogs?limit=1', { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch('/api/jobs?limit=1', { headers: { 'Authorization': `Bearer ${token}` } })
      ])

      const [users, blogs, jobs] = await Promise.all([
        usersRes.ok ? usersRes.json() : { data: [], pagination: { totalItems: 0 } },
        blogsRes.ok ? blogsRes.json() : { data: [], pagination: { totalItems: 0 } },
        jobsRes.ok ? jobsRes.json() : { data: [], pagination: { totalItems: 0 } }
      ])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl shadow-lg p-6 text-white mb-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-100 text-lg">Welcome back! Here&apos;s what&apos;s happening with your platform.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6">
        {isLoading ? (
          Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-lg border border-gray-100 p-6 animate-pulse">
          ...
          stats.map((stat) => (
          <div key={stat.name} className="group bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-lg hover:shadow-xl border border-gray-100 p-6 transition-all duration-300 hover:-translate-y-1">
            ...
            {/* Decorative element */}
            <div className={`mt-4 h-1 rounded-full bg-gradient-to-r ${stat.color} opacity-20 group-hover:opacity-40 transition-opacity`}></div>
          </div>
        ))
        )}
      </div>
      ...
      <LazyWrapper fallback={<div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><div className="bg-gray-200 rounded-xl h-64 animate-pulse"></div><div className="bg-gray-200 rounded-xl h-64 animate-pulse"></div></div>}>
        ...
        <div className="bg-gradient-to-br from-white to-blue-50 rounded-xl shadow-lg border border-blue-100 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-600">Track user acquisition over time</p>
            </div>
            <div className="flex items-center space-x-2 bg-blue-100 px-3 py-2 rounded-full">
              <LineChart className="h-5 w-5 text-blue-600" />
              <span className="text-sm font-medium text-blue-700">Last 30 days</span>
            </div>
          </div>
          <div className="h-64 flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-blue-100">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 mt-1">Real-time user growth visualization</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-white to-purple-50 rounded-xl shadow-lg border border-purple-100 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-xl font-bold text-gray-900">Content Distribution</h3>
              <p className="text-sm text-gray-600">Content breakdown by category</p>
            </div>
          ...
          <div className="space-y-4">
            {[
              { label: "Blog Posts", percentage: 45, color: "bg-blue-500", bgColor: "bg-blue-50" },
              { label: "Job Listings", percentage: 30, color: "bg-green-500", bgColor: "bg-green-50" },
              { label: "Exam Updates", percentage: 15, color: "bg-purple-500", bgColor: "bg-purple-50" },
              { label: "Discounts", percentage: 10, color: "bg-orange-500", bgColor: "bg-orange-50" }
            ].map((item, index) => (
        ...
        <div className="bg-gradient-to-br from-white to-green-50 rounded-xl shadow-lg border border-green-100 p-6">
          ...
          <div className="space-y-4">
            {systemStatus.map((item) => (
              <div key={item.name} className="group flex items-center justify-between p-3 rounded-lg hover:bg-white/50 transition-colors">
                <div className="flex items-center">
                  <div className={`p-2 rounded-lg ${
                    item.status === "operational" ? "bg-green-100" : "bg-yellow-100"
                  } group-hover:scale-110 transition-transform`}>
                    <item.icon className={`h-4 w-4 ${
                      item.status === "operational" ? "text-green-600" : "text-yellow-600"
                    }`} />
                  </div>
                  <span className="text-sm font-medium text-gray-700 ml-3">{item.name}</span>
                </div>
        ...
        <div className="bg-gradient-to-br from-white to-blue-50 rounded-xl shadow-lg border border-blue-100 p-6">
          ...
          <div className="space-y-4">
            {recentActivities.map((activity) => (
              <div key={activity.id} className="group flex items-start space-x-3 p-3 rounded-lg hover:bg-white/50 transition-colors">
                <div className={`p-2 rounded-lg ${activity.color.replace('text-', 'bg-').replace('-500', '-100')} group-hover:scale-110 transition-transform`}>
          ...
          <button className="mt-4 w-full text-center text-sm text-blue-600 hover:text-blue-800 font-medium py-2 rounded-lg hover:bg-blue-50 transition-colors" aria-label="View all activity">
            View all activity →
          </button>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-lg border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            >
              <div className={`p-3 rounded-xl bg-${action.color}-100 group-hover:bg-${action.color}-200 transition-colors mr-4`}>
              ...
              <div className={`w-2 h-2 rounded-full bg-${action.color}-400 group-hover:scale-125 transition-transform`}></div>
            </button>
          ))}
        </div>
      </div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        onClose()
        setFormData({ name: '', email: '', role: 'Student', password: '', confirmPassword: '' })
      } else {
...
function NewBlogModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  const [formData, setFormData] = useState({
    title: '',
    summary: '',
    content: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        onClose()
        setFormData({ title: '', summary: '', content: '', category: 'technology', tags: '', published: false })
      } else {
        const data = await res.json()
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <h2 className="text-2xl font-bold mb-6 text-gray-900">Create New Blog Post</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        onClose()
        setFormData({
          title: '', company: '', description: '', type: 'full-time', category: 'software',
          location: '', remote: false, salary: { min: 0, max: 0, currency: 'INR' },
          requirements: '', skills: '', experience: { min: 0, max: 0 },
          applicationDeadline: '', applicationLink: '', contactEmail: '',
...
function ScheduleEventModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({
          ...
          organizer: '', maxAttendees: '', registrationRequired: false, isActive: true
        })
      } else {
        const data = await res.json()
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <h2 className="text-2xl font-bold mb-6 text-gray-900">Schedule New Event</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
